...
#else
    status = nxd_mqtt_client_connect(mqtt_client_ptr, &server_address, NXD_MQTT_TLS_PORT,
    		 NX_AZURE_IOT_MQTT_KEEP_ALIVE,NX_FALSE, wait_option);
#endif
...
az_span policy_name = AZ_SPAN_LITERAL_FROM_STR(NX_AZURE_IOT_PROVISIONING_CLIENT_POLICY_NAME);
    ...
    status = nx_azure_iot_buffer_allocate(prov_client_ptr -> nx_azure_iot_ptr,
                                          ...
                                          &buffer_context);
    printf("nx_azure_iot_buffer_allocate: BUFFER size %d\r\n", buffer_size);
    if (status)
    {
        LogError(LogLiteralArgs("IoTProvisioning client sas token fail: BUFFER ALLOCATE FAIL"));
        printf("IoTProvisioning client sas token fail: BUFFER ALLOCATE FAI\r\n");
        return(status);
    }
    ...
    if (az_result_failed(core_result))
    {
        ...
        printf("IoTProvisioning failed failed to get signature with error status: %d\r\n", core_result);
        return(NX_AZURE_IOT_SDK_CORE_ERROR);
    }
    ...
    if (status)
    {
        ...
        printf("IoTProvisioning failed to encoded hash\r\n");
        nx_azure_iot_buffer_free(buffer_context);

        return(status);
    }
    printf("22expiry_time_secs %ld\r\n", expiry_time_secs);
    ...
    if (az_result_failed(core_result))
    {
        ...
        nx_azure_iot_buffer_free(buffer_context);
        printf("IoTProvisioning failed to generate token with error : %d\r\n", core_result);
        return(NX_AZURE_IOT_SDK_CORE_ERROR);
    }
    printf("resource_mqtt_sas_token %s\n",resource_ptr -> resource_mqtt_sas_token);
    nx_azure_iot_buffer_free(buffer_context);
...
UINT old_threshold;
UINT status;
printf("nx_azure_iot_provisioning_client_register\r\n");
    if ((prov_client_ptr == NX_NULL) || (prov_client_ptr -> nx_azure_iot_ptr == NX_NULL))
    {
    ...
    //printf("2. nx_azure_iot_provisioning_client_register\r\n");
    /* Obtain the mutex.  */
    tx_mutex_get(prov_client_ptr -> nx_azure_iot_ptr -> nx_azure_iot_mutex_ptr, NX_WAIT_FOREVER);

    if (prov_client_ptr -> nx_azure_iot_provisioning_client_state == NX_AZURE_IOT_PROVISIONING_CLIENT_STATUS_INIT)
    {
    	//printf("3. nx_azure_iot_provisioning_client_register\r\n");
        /* Update state in user thread under mutex.  */
        nx_azure_iot_provisioning_client_update_state(prov_client_ptr, NX_AZURE_IOT_PENDING);
        //printf("4. nx_azure_iot_provisioning_client_register\r\n");
        /* Trigger workflow.  */
        status = nx_cloud_module_event_set(&(prov_client_ptr -> nx_azure_iot_ptr -> nx_azure_iot_cloud_module),
                                           NX_AZURE_IOT_PROVISIONING_CLIENT_CONNECT_EVENT);
        //printf("5.nx_azure_iot_provisioning_client_register\r\n");
        if (status)
        {
        	printf("6.nx_azure_iot_provisioning_client_register\r\n");
            nx_azure_iot_provisioning_client_update_state(prov_client_ptr, status);
        }
    }
    ...
    if (wait_option)
    {
        if (prov_client_ptr -> nx_azure_iot_provisioning_client_state > NX_AZURE_IOT_PROVISIONING_CLIENT_STATUS_INIT &&
             prov_client_ptr -> nx_azure_iot_provisioning_client_state < NX_AZURE_IOT_PROVISIONING_CLIENT_STATUS_DONE)
        {
        	//printf("7.nx_azure_iot_provisioning_client_register\r\n");
...
#endif
            printf("10.nx_azure_iot_provisioning_client_register\r\n");
            /* Release the mutex.  */
            tx_mutex_put(prov_client_ptr -> nx_azure_iot_ptr -> nx_azure_iot_mutex_ptr);
            printf("11.nx_azure_iot_provisioning_client_register\r\n");
            tx_thread_sleep(wait_option);
            printf("12.nx_azure_iot_provisioning_client_register\r\n");
#if 0 /*YUN*/
            /* Obtain the mutex.  */
            tx_mutex_get(prov_client_ptr -> nx_azure_iot_ptr -> nx_azure_iot_mutex_ptr, NX_WAIT_FOREVER);
            printf("13.nx_azure_iot_provisioning_client_register\r\n");

            /* Restore preemption.  */
            tx_thread_preemption_change(tx_thread_identify(), old_threshold, &old_threshold);
            printf("14.nx_azure_iot_provisioning_client_register\r\n");
#endif
            nx_azure_iot_provisioning_client_thread_dequeue(prov_client_ptr, &thread_list);
            //printf("15.nx_azure_iot_provisioning_client_register\r\n");
        }
    }
    //printf("8.nx_azure_iot_provisioning_client_register\r\n");
    if (prov_client_ptr -> nx_azure_iot_provisioning_client_state > NX_AZURE_IOT_PROVISIONING_CLIENT_STATUS_INIT &&
         prov_client_ptr -> nx_azure_iot_provisioning_client_state < NX_AZURE_IOT_PROVISIONING_CLIENT_STATUS_DONE)
    {
    ...
    printf("9.nx_azure_iot_provisioning_client_register\r\n");
    /* Release the mutex.  */
    tx_mutex_put(prov_client_ptr -> nx_azure_iot_ptr -> nx_azure_iot_mutex_ptr);
...
UINT status;
        ...
        (symmetric_key == NX_NULL) || (symmetric_key_length == 0))
    {
        ...
        printf("IoTProvisioning client symmetric key fail: Invalid argument\n");
        return(NX_AZURE_IOT_INVALID_PARAMETER);
    }
    ...
    if (status)
    {
        ...
        printf("IoTProvisioning client symmetric key fail status: %d\n", status);
        return(status);
    }
    printf("1 expiry_time_secs: %d\n", expiry_time_secs);
...
printf("nx_azure_iot_provisioning_client_iothub_device_info_get\r\n");
    if ((prov_client_ptr == NX_NULL) || (prov_client_ptr -> nx_azure_iot_ptr == NX_NULL) ||
...
{
	printf("nx_azure_iot_provisioning_client_registration_payload_set\r\n");
    if ((prov_client_ptr == NX_NULL) || (prov_client_ptr -> nx_azure_iot_ptr == NX_NULL))
    {
