...
static az_log_classification const classifications[] = {AZ_LOG_IOT_AZURERTOS,
                                                        ...
                                                        /*,_az_LOG_END_OF_LIST*/};/*YUN*/

    //_az_log_set_classifications(classifications); /*YUN*/
...
#else
    status = nx_packet_allocate(client_ptr -> nxd_mqtt_client_packet_pool_ptr, packet_pptr, NX_IPv6_TCP_PACKET,
                                            TX_WAIT_FOREVER);
#endif
    if (status)
    {
...
const NX_CRYPTO_METHOD *hmac_sha_256_crypto_method = NX_NULL;
    ...
    {
    	xprintf("hmac_sha_256_crypto_method == NX_NULL\n");
    ...
    //xprintf("#####3333#####1234567890 status %d\n",status);
    return(status);
}
...
UINT binary_key_buf_size;
    ...
    if (status)
    {
        ...
        xprintf("Failed to base64 decode\n");
        return(status);
    }
    ...
    if ((hash_buf_size + encoded_hash_buf_size) > buffer_len)
    {
        ...
        xprintf("Failed to not enough memory\n");
        return(NX_AZURE_IOT_INSUFFICIENT_BUFFER_SPACE);
    }

    hash_buf = buffer_ptr + binary_key_buf_size;
    //xprintf("binary_key_buf_size %d\n", binary_key_buf_size);
    ...
    if (status)
    {
        ...
        xprintf("Failed to get hash256 status %d\n", status);
        return(status);
    }
    ...
    if (status)
    {
        ...
        xprintf("Failed to base64 encode\n");
        return(status);
    }
