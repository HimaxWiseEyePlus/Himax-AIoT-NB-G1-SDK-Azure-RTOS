...
UINT nx_azure_iot_hub_client_connection_status_callback_set(NX_AZURE_IOT_HUB_CLIENT *hub_client_ptr,
                                                            VOID (*connection_status_cb)(
                                                                  ...
                                                                  UINT status))
{
	 printf("nx_azure_iot_hub_client_connection_status_callback_set\r\n");
...
#else
    nxd_mqtt_client_connect(mqtt_client_ptr, &server_address, NXD_MQTT_TLS_PORT,
    						NX_AZURE_IOT_MQTT_KEEP_ALIVE, clean_session, wait_option);
#endif
    /* Obtain the mutex.  */
    tx_mutex_get(hub_client_ptr -> nx_azure_iot_ptr -> nx_azure_iot_mutex_ptr, TX_WAIT_FOREVER);
...
{
	printf("nx_azure_iot_hub_client_symmetric_key_set\r\n");
    if ((hub_client_ptr == NX_NULL)  || (hub_client_ptr -> nx_azure_iot_ptr == NX_NULL) ||
...
{
	printf("nx_azure_iot_hub_client_model_id_set\r\n");
    if ((hub_client_ptr == NX_NULL)  || (hub_client_ptr -> nx_azure_iot_ptr == NX_NULL) ||
